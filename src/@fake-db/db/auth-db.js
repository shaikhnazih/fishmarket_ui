...
mock.onPost('/api/auth1').reply((config) => {
    ...
    const error = {
        ...
        password: user && user.password === password ? null : 'Check your password11111'
    };
...
mock.onGet('/api/auth1/access-token').reply((config) => {
    const data = JSON.parse(config.data);
...
mock.onPost('/api/auth1/register').reply((request) => {
...
mock.onPost('/api/auth1/user/update').reply((config) => {
    const data = JSON.parse(config.data);
    const {user} = data;
